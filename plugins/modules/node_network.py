...
class NodeNetworkModule(OpenStackModule):
    ...
    def run(self):
        ...
        network_port = self._find_network_port(node.name, network, port, trunk)
    ...
    def _find_network_port(self, node_name, network, network_port, trunk):
        port = None
    ...
    def _find_matching_baremetal_port(self, baremetal_ports,
        ...
        for bp in baremetal_ports:
            if mac_address and bp.address != mac_address:
                continue
            if ("tenant_vif_port_id" in bp.internal_info and
                    bp.internal_info["tenant_vif_port_id"] == network_port.id):
                return bp
